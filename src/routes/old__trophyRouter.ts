...
function loadServiceDoc(file: string): any {
  ...
  if (!fs.existsSync(absFile)) {
    const error: Error = new Error(`${absFile} not found.`);
...
const genTrophies = async (req: express.Request, res: express.Response) => {
  ...
  const data = JSON.parse(fs.readFileSync(TROPHY_DATA_FILE, 'UTF-8'));
 ...
 * @param res
 */
const insertScore = async (req: express.Request, res: express.Response) => {
  log.debug(__filename, req.url, 'Handling request -> ' + req.path);
  let score: Score;
 ...
 * @param res
 */
const updateScore = async (req: express.Request, res: express.Response) => {
  log.debug(__filename, req.url, 'Handling request -> ' + req.path);
  let score = req.body;
 ...
 * @param res - express.Response
 */
const deleteScore = async (req: express.Request, res: express.Response) => {
  log.debug(__filename, req.url, 'Handling request -> ' + req.path);
  const query: any = { id: req.params.scoreId };
