 ...
 * @param res - express response
 */
function getTopScoresByMaze(mazeId, teamIds, teamGames, req) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            while (dupes) {
                docs = yield dbMan.getDocuments(colName, query, sort, projection, pageSize, pageNum);
                if (docs.length > 1) {
                    if (docs[0].botId === docs[1].botId && docs[0].totalScore === docs[1].totalScore) {
                        yield dbMan.deleteDocument(config.MONGO_COL_SCORES, { id: docs[1].id });
                    }
                    else if (docs.length > 2 && docs[1].botId === docs[2].botId && docs[1].totalScore === docs[2].totalScore) {
                        yield dbMan.deleteDocument(config.MONGO_COL_SCORES, { id: docs[2].id });
                    }
                    else {
                        dupes = false;
                    }
...
exports.getTopScoresByMaze = getTopScoresByMaze;
/**
 * Returns all documents from the given collection that match the query parameters. If no
 * query parameters are given, all documents will be returned (be careful!)
 *
